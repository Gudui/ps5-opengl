...
case "$requested_test" in
    core33-alpha-blend)
        gate_object=egl_public_core33_alpha_blend.o
        ;;
    core33-sampler-state)
        gate_object=egl_public_core33_sampler_state.o
        ;;
    core33-texture-2d)
        gate_object=egl_public_core33_texture_2d.o
        ;;
    ...
    egl_public_*)
        ...
        ;;
    *)
        printf 'usage: %s {--list|core33-alpha-blend|core33-sampler-state|core33-texture-2d|core33-uniform-matrix|core33-texture-rectangle|core33-texture-rgtc|core33-depth-texture|egl_public_<gate>[.o]}\n' "$0" >&2
        exit 2
        ;;
esac

[[ $gate_object == egl_public_core33_triangle.o || $gate_object == egl_public_core33_indexed_triangle.o || $gate_object == egl_public_core33_uniform_matrix.o || $gate_object == egl_public_core33_texture_2d.o || $gate_object == egl_public_core33_sampler_state.o || $gate_object == egl_public_core33_alpha_blend.o ]] || [[ $gate_object =~ ^egl_public_core33_(imgui(_tv|_lifecycle|_benchmark)?|nanovg|sokol(_cube)?)\.o$ ]] || grep -qxF "${gate_object}:" < <(
    grep -oE '^egl_public_[A-Za-z0-9_]+\.o:' "$root/tests/ps5/Makefile"
) || {
...
if [[ $gate_object == egl_public_core33_triangle.o || $gate_object == egl_public_core33_indexed_triangle.o || $gate_object == egl_public_core33_uniform_matrix.o || $gate_object == egl_public_core33_texture_2d.o || $gate_object == egl_public_core33_sampler_state.o || $gate_object == egl_public_core33_alpha_blend.o ]]; then
    ...
    elif [[ $gate_object == egl_public_core33_texture_2d.o ]]; then
        triangle_defines=(-DPS5_NATIVE_TEXTURE_2D=1)
    elif [[ $gate_object == egl_public_core33_sampler_state.o ]]; then
        triangle_defines=(-DPS5_NATIVE_SAMPLER_STATE=1)
    elif [[ $gate_object == egl_public_core33_alpha_blend.o ]]; then
        triangle_defines=(-DPS5_NATIVE_ALPHA_BLEND=1)
    fi
...
{
    printf 'SEARCH_DIR("%s")\n' "$sdk/target/lib"
    if [[ $gate_object == egl_public_core33_imgui*.o || $gate_object == egl_public_core33_triangle.o || $gate_object == egl_public_core33_indexed_triangle.o || $gate_object == egl_public_core33_uniform_matrix.o || $gate_object == egl_public_core33_texture_2d.o || $gate_object == egl_public_core33_sampler_state.o || $gate_object == egl_public_core33_alpha_blend.o ]]; then
        printf 'SEARCH_DIR("%s")\n' "$prefix/lib"
    fi
...
python3 "$root/tools/write-folder-manifest.py" --root "$app/dist" --title-id "$title_id"
